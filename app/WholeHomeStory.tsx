...
/**
 * The whole-home story: the hero, the manifesto, the four systems, the case
 * feature, the collection, the process and the consultation.
 *
 * It is rendered at two addresses. "/" is where visitors arrive, and
 * /services/whole-home is where the Experience menu sends anyone looking for
 * the connected-home service — the service is the story, so the page is the
 * same one rather than a second, thinner telling of it that would drift out of
 * step the first time either was edited.
 *
 * Two URLs serving one body is duplicate content, so the service route
 * declares "/" as its canonical. That keeps the menu entry working while
 * pointing every ranking signal at the homepage.
 */
...
export function WholeHomeStory() {
  return (
    <>
